...
|---------|----------|---------------|----------|----------|
| TEST-1-1-1.1 | N-1.1 | `app/src/hello.test.ts` | `hello returns Hello Claude!` | The return value of `hello()` is `"Hello Claude!"` |
...
E2E not needed (reason: a single function with no screen. It has no boundary to integrate).
...
|----|------|------|------|------|
| S-1 | Machine scan | app/src | `command grep -rn 'TODO' app/src` | 0 lines |
...
|----|--------|------|------|
| S-1 | qa | `command grep -rn 'TODO' app/src` → 0 lines | 🟢 |
