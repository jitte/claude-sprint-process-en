...
The definitions of metrics and commands are in `docs/06_process/spec-metrics.md`. The definitions of the indicators are also in the `/spec-coverage` skill. This section covers only how to read the output when looking for places where the structural or content evaluation is low.
